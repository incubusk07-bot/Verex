...
import { DEFAULT_CURRENCY, detectCurrency, type CurrencyConfig } from "@/services/currency";
import type {
  CollectionItem,
  GameId,
  MatchedCard,
...
import { makeId, setActiveCurrency } from "@/utils/format";
...
interface PersistedState {
  ...
  captureHintSeen: boolean;
  currency: CurrencyConfig;
  detectionStrictness: DetectionStrictness;
}
...
export const [AppProvider, useApp] = createContextHook(() => {
  ...
  const [captureHintSeen, setCaptureHintSeen] = useState<boolean>(false);
  const [currency, setCurrency] = useState<CurrencyConfig>(DEFAULT_CURRENCY);
  const [detectionStrictness, setDetectionStrictness] = useState<DetectionStrictness>("balanced");
    ...
    (async () => {
      try {
        ...
        if (raw) {
          ...
          setCaptureHintSeen(state.captureHintSeen === true);
          setDetectionStrictness(
            state.detectionStrictness === "lenient" || state.detectionStrictness === "strict"
              ? state.detectionStrictness
              : "balanced",
          );
          if (state.currency && typeof state.currency.rate === "number" && state.currency.code) {
  ...
  // ---------- currency auto-detection (IP-based, refreshed every 12h) ----------
  useEffect(() => {
    if (!hydrated) return;
    const ageMs = Date.now() - new Date(currency.detectedAt).getTime();
  ...
  const dismissCaptureHint = useCallback(() => setCaptureHintSeen(true), []);
  const resetCaptureHint = useCallback(() => setCaptureHintSeen(false), []);
