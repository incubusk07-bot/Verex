...
/** How aggressively the pipeline calls a card fake vs. original. */
export type DetectionStrictness = "lenient" | "balanced" | "strict";

export interface DetectionProfile {
  /** % database-match confidence required to accept the reference card. */
  matchThreshold: number;
  /** Token-overlap ratio required for the OCR cross-check to pass. */
  textSimPass: number;
  /** Below this token-overlap ratio the text mismatch counts toward counterfeit. */
  textSimFail: number;
  /** Vision print-sharpness (1-10) below this counts as a structural fail. */
  sharpnessMin: number;
}

export const DETECTION_PROFILES: Record<DetectionStrictness, DetectionProfile> = {
  lenient: { matchThreshold: 78, textSimPass: 0.45, textSimFail: 0.28, sharpnessMin: 4 },
  balanced: { matchThreshold: 85, textSimPass: 0.55, textSimFail: 0.35, sharpnessMin: 5 },
  strict: { matchThreshold: 90, textSimPass: 0.65, textSimFail: 0.45, sharpnessMin: 6 },
};

export const STRICTNESS_META: { id: DetectionStrictness; label: string; sub: string }[] = [
  { id: "lenient", label: "Lenient", sub: "Fewer false alarms — accepts weaker matches" },
  { id: "balanced", label: "Balanced", sub: "Recommended default for most cards" },
  { id: "strict", label: "Strict", sub: "Hardest to pass — for high-value deals" },
];
