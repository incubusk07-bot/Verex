...
import { CheckCircle2, ImageIcon, Lightbulb, Timer, TimerOff, X, Zap, ZapOff } from "lucide-react-native";
...
export default function ScanScreen() {
  ...
  const { autoCapture, setAutoCapture, captureHintSeen, dismissCaptureHint } = useApp();
  ...
  useEffect(() => {
    if (!focused || !cameraReady || !autoCapture || capturing || !permission?.granted || !captureHintSeen) {
      progressAnim.setValue(0);
      return;
    }
    progressAnim.setValue(0);
  ...
  }, [focused, cameraReady, autoCapture, capturing, permission?.granted, captureHintSeen, capture, progressAnim]);
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.permissionWrap}>
          <Image
            source={require("@/assets/images/brand/dragon-card.png")}
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.cameraWrap}>
        ...
        {!captureHintSeen ? (
          <View style={styles.hintCard} testID="capture-hint">
            <View style={styles.hintHeader}>
              <Lightbulb size={17} color={colors.goldOnDark} />
              <Text style={styles.hintTitle}>How to capture a great scan</Text>
              <Pressable
                testID="capture-hint-close"
                hitSlop={10}
                onPress={() => {
                  void tapHaptic("select");
                  dismissCaptureHint();
              ...
              >
                <X size={18} color={colors.slateOnDark} />
              </Pressable>
            </View>
            {[
              "Fill the gold frame with the whole card",
              "Use even, diffused light — avoid glare on the surface",
              "Place the card on a plain, dark background",
              "Hold steady — auto-capture fires once you're aligned",
            ].map((tip) => (
              <View key={tip} style={styles.hintRow}>
                <CheckCircle2 size={14} color={colors.goldSoftOnDark} />
                <Text style={styles.hintText}>{tip}</Text>
              </View>
            ))}
            <Pressable
              testID="capture-hint-got-it"
              onPress={() => {
                void tapHaptic("select");
                dismissCaptureHint();
              }}
              style={({ pressed }) => [styles.hintButton, { opacity: pressed ? 0.85 : 1 }]}
            >
              <Text style={styles.hintButtonText}>Got it — start scanning</Text>
            </Pressable>
          </View>
        ) : null}
