...
function buildSignals(params: {
  ...
  const { card, matchConfidence, textSim, ocr, vision, profile } = params;
  const signals: AuthenticitySignal[] = [];

  signals.push({
    id: "reference_match",
    label: "Reference database match",
    passed: card ? matchConfidence >= profile.matchThreshold : false,
    detail: card
  ...
  if (card) {
    signals.push({
      ...
      passed: ocr.ok ? textSim >= profile.textSimPass : null,
      detail: ocr.ok
  ...
  if (vision) {
    signals.push({
      ...
      passed: vision.printSharpness >= profile.sharpnessMin + 1,
...
function decideVerdict(params: {
  ...
  const { card, matchConfidence, textSim, vision, profile } = params;
  if (!card || matchConfidence < profile.matchThreshold) return "inconclusive";

  const flags = vision?.authenticityFlags.length ?? 0;
  const holoFail = vision?.holoVarianceOk === false;
  const sharpnessFail = vision !== null && vision.printSharpness < profile.sharpnessMin;
  const structuralFails = (flags >= 2 ? 1 : 0) + (holoFail ? 1 : 0) + (sharpnessFail ? 1 : 0);

  if (textSim < profile.textSimFail || structuralFails >= 2) return "likely_counterfeit";
  if (textSim >= profile.textSimPass && flags === 0 && !holoFail && !sharpnessFail)
...
export async function analyzeScan(
  ...
  profile: DetectionProfile = DETECTION_PROFILES.balanced,
  ...
  const card = top && confidence >= profile.matchThreshold ? top.card : null;
  ...
  const signals = buildSignals({ card, matchConfidence: confidence, textSim, ocr, vision, profile });
  const pillars = computePillars(vision);
  const verdict = quality.ok
    ? decideVerdict({ card, matchConfidence: confidence, textSim, vision, profile })
    : "inconclusive";
